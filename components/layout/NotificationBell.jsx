...
export default function NotificationBell({ user }) {
  ...
  if (!user) return null
  const enablePush = async () => { setEnabling(true); try { await enableCustomerPush(); setPushEnabled(true); toast('success', 'Browser notifications are enabled on this device.', 'Notifications enabled') } catch (error) { toast('error', error.message, 'Unable to enable notifications') } finally { setEnabling(false) } }
