...
export default function CustomOrder({ variant = 'default' }) {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'printType' && value === 'None') removeImage()
  }
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        toast('success', 'Your design image has been uploaded.', 'Image uploaded')
      }
  ...
  const handleAddToCart = () => {
    // Store the customer design preview with the custom cart item.
    const customProduct = {
      id: `custom-${Date.now()}`,
    ...
    toast('success', 'Your custom shirt has been added to the cart.', 'Added to cart')
  }
    ...
    <div className={variant === 'home' ? '' : 'mt-12 border-t-2 border-black pt-10'}>
      <div className={`bg-white rounded-3xl p-6 md:p-8 ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          {/* Custom Text */}
          {form.printType !== 'None' && <div className="md:col-span-2">
            <label className="text-xs font-bold uppercase tracking-widest text-black block mb-1.5">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 pt-4 border-t-2 border-gray-100">
          ...
          <button
            ...
            className="w-full rounded-xl bg-black px-10 py-3 text-sm font-bold uppercase tracking-widest text-white transition-all duration-300 hover:bg-gray-800 hover:shadow-lg active:scale-95 sm:w-auto"
          >
